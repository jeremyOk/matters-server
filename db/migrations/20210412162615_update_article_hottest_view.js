...
exports.up = async (knex) =>
  ...
  with original_score as (
    ...
    (
        ...
        and a.id not in (select entrance_id from collection where article_id = 8079 )
  ...
  (
    ...
    greatest(
        (select max_efficiency from original_score)*coalesce(${donation_decay_factor}^(extract(epoch from now()-t2.latest_transaction)::decimal/3600), 0)/t2.count_normal_transaction,
    ...
    where t1.id not in (select entrance_id from collection where article_id = 8079 ) --to block all articles in the Complaint Area
